...
def home(request):
    ...
    for file_database in all_file_databases:
        table_rows.append(
            (
                file_database['id'], file_database['path'],
                file_database['collection_count'], file_database['meta']
            )
        )
    ...
    if len(table_rows) > 0:
        file_databases_table = DataTableView(
            column_names=('ID', 'Path', 'Collection Count', 'Meta'),
            rows=table_rows,
            searching=False,
            orderClasses=False,
    ...
    context = {
        ...
        'file_databases_table': file_databases_table,
    }
...
def upload_files(request, workspace):
    ...
    # default options
    database_select_options = [(database.id, database.id) for database in all_databases]
    ...
    if request.POST and 'add-button' in request.POST:
        ...
        if not has_errors:
            add_uploaded_files(workspace=workspace, session=session, name=name, database_id=selected_database_id,
